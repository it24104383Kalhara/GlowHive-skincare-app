...
const CheckoutScreen = ({ navigation }) => {
  ...
  const validateAll = () => {
    ...
    else if (!/^[0-9]{10}$/.test(contactNumber.replace(/[\s\-]/g, ''))) newErrors.contactNumber = 'Enter a valid phone number (10 digits)';
    if (!address.trim()) newErrors.address = 'Address is required';
    else if (address.trim().length < 10) newErrors.address = 'Please enter a complete address';
    if (!postalCode.trim()) newErrors.postalCode = 'Postal code is required';
    else if (!/^[0-9]{5}$/.test(postalCode.trim())) newErrors.postalCode = 'Must be a 5-digit postal code';
    setErrors(newErrors);
