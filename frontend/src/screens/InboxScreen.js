...
const deleteConversation = async (conversationId) => {
  ...
  const filteredConversations = useMemo(() => {
    ...
    if (filterStatus === 'replied') {
      filtered = filtered.filter(conv => {
        ...
        // If current user is the last sender, it means we replied
        return String(lastSenderId) === String(user._id);
      });
    } else if (filterStatus === 'unreplied') {
      filtered = filtered.filter(conv => {
        const lastSenderId = conv.lastMessageSenderId;
        // If no messages yet, or last sender is the other person, it's unreplied
        if (!lastSenderId) return true;
        return String(lastSenderId) !== String(user._id);
      });
    }
  ...
  }, [sortedConversations, searchQuery, user, filterStatus]);
